...
import asyncdispatch, json, os, times, strformat, strutils
...
proc compareLLMs() {.async.} =
  ...
  # Query all providers sequentially (to avoid closure capture issues)
  var results: seq[tuple[name: string, response: string, duration: float]] = @[]
  ...
  for i in 0..<clients.len:
    ...
    try:
      ...
      results.add((name: name, response: response, duration: duration))
    except Exception as e:
      results.add((name: name, response: fmt"Error: {e.msg}", duration: 0.0))
